import novo from "./Novo-icon.svg";
import style from "./CardCarrossel.module.scss"

const CardCarrossel = ({children, preco, image}) => {
  const LIMIT = 20

  const aboveLimit = children.length > LIMIT
  const dotsOrEmpty = aboveLimit ? '...' : ''
  const nome = children.substring(0, LIMIT) + dotsOrEmpty
    return (
